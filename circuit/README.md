...
##### 超忘れてた！エンコーダ！！！
[ENX 10 EASY](https://www.maxongroup.co.jp/medias/sys_master/root/8836894752798/enx-easy-en-Nov-11.pdf) データシートの P.17, P.24 あたりが有能。
...
- スライドスイッチ : [SSAJ120100](https://akizukidenshi.com/catalog/g/gP-13989/) [Pch MOSFET CXDM4060P TR PBFREE](https://www.digikey.jp/ja/products/detail/central-semiconductor-corp/CXDM4060P-TR-PBFREE/4807590)
...
- [ ] PA2 ~ 5 を無効にして左右の取得値からモータを制御する。
- [ ] そのために PA2 ~ 5 を切り捨てて左右の値を足し算するプログラムの作成。
- [ ] そのために PWM 制御するピンにタイマを割り当てる。
...
メモ書き
IN0, 7, 8, 11 // 今後使うかわからんけど一応記述しておく。
...
LED_R, G, B を追加
TIM6 を追加
SW3 を押すことで UART の出力をループしたりしなかったりするようにした。
ループするかしないかによって FullColorLED の色が変化するようにした。
SW3 の Read は TIM6 を使って、while 中でも loopflg が変化するようにした。
まだプログラム上で作っただけなので、実機で試しておらず、本当にこのプログラムで動くのかは不明。
又、TIM6 の Configrations はデフォルトのまま（NIVC のみ Enable に変更済み）なので実装する前に改めて TIM6 の設定を見直すことを勧める。

2022/03/29 **:**
DRV1/2_PH/EN を追加。
TIM4, TIM6, TIM7 を追加し、割り込み周期の調整。
