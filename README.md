...
- [ ] センサバーの長さ・トレッド・ホイールベース
 - [ ] センサバーの長さ > 曲線センサは下記思考で自動的に長さが決まる。直線センサは車体と同じくらいの長さ。
 - [ ] トレッド > 100 - 13 ∵ トレッドが 100 でタイヤ幅が 13 というのが先に決まったから。
 - [ ] ホイールベース > 極めて短く ∵ タイヤの軸方向のモーメントを小さくするため。 => 24 mm ∵ タイヤ径から少し余裕を持たせたホイールベースを考えた結果
 - 以下思考
 - ホイールシャフトの中点を中心に円形にセンサを配置したい。そうすればセンサの値で角度がわかりやすい。直線センサに対してもこれを適応してもいい。ただ直線センサは距離が遠くなるのでほとんどまっすぐになりそう。
...
- [x] タイヤ径・幅
 - [x] 径 > 22 (mm) (直径) ∵ タイヤ径を決めないとモータが決まらず、モータ決めないとタイヤ径が決まらない状況になって、井口先輩も下鳥先輩もノムさんもこのくらいにしてるから。
 - [x] 幅 > 13 (mm) ∵ シートが貼れる幅。
...
- [x] ギヤ比 > 
- スパーギア > M0.3 Z64 AD19.8
- ピニオンギア > M0.3 Z42 AD13.2
- ギヤ比 > 64 / 42 ≒ 1.523809523089523809... // 一番の理想はこれなんだが、互いに素にした方がよいのでギア比は 64 / 41 にする。
- ~~ギヤ比って回転数で決めるのかトルク比で決めるのか、どっちで決めた方がいいのか... maxon motor のカタログの P54 くらいを見てヒントを探す。~~
- ~~回転数とトルクをまとめると...~~
- ~~最高速度 : 8.5 m/s~~
...
- Excel でギア比とモータの性能を変数にしたシートを作成して、回転数とトルクが必要以上でありつつ、許容回転数をオーバーしないようなパターンを探したら、上記のモータとギヤ比に決まった。
